...
Kestrel separates five stages. `terraform.parser` loads `terraform show -json` output into immutable resource-change models and applies Terraform sensitivity masks plus secret-like key redaction at the boundary. `risk.rules` evaluates those models locally and produces explainable findings before any model call. `agent.planner` runs the bounded observe-decide-execute cycle. `aws.client` wraps boto3 sessions configured with profile and region, while AWS tools catch unavailable credentials without leaking details. `reporting` renders the common report as plain terminal text or JSON.
